...
class DecisionStump:
    def __init__(self, n_caracteristicas):
        ...
        self.umbral = np.random.uniform() # Inicializar el umbral a None. Este será determinado durante el entrenamiento.
    ...
    def fit(self, X, Y, verbose=False):
        ...
        for t in range(self.T):
            ...
            for a in range(self.A):
                ...
                clf.caracteristica_index = np.random.randint(0, n_caracteristicas)
                clf.umbral = np.random.uniform()
                clf.polaridad = np.random.choice([1, -1])
...
def generarGraficas(T_values, A_values, train_accuracies, test_accuracies, training_times):
...
def main():
    ...
    # train_accuracy, test_accuracy, training_time = entrenamiento(9, 10, 20)

    # Define los rangos de valores para T y A que deseas probar
    #T_values = [10, 10, 10, 10]# 10, 20, 5, 30]
    T_values = [10, 20, 30, 40]
    A_values = [5, 10, 20, 30]# 40, 40, 30, 30] 
    ...
    for digito in digitos: # Tarea 1D
        ...
        generarGraficas(T_values, A_values, train_accuracies_matrizAux, test_accuracies_matrizAux, training_times_matrizAux)
